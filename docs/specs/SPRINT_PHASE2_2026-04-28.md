...
| 22 | **PY-V2-action** — Python 3.14 compat lane | Phase 2 | P4 | ✅ COMPLETED locally | None | CI job `python-314-compat` runs as allowed-to-fail, `scripts/check_pydantic_migration_guard.py --json -> ok: true`, and `tests/scripts/test_pydantic_migration_guard.py` guards the lane plus migration-plan requirement; promote to required only after Python 3.14 compatibility criteria in `docs/engineering/PYDANTIC_V2_MIGRATION_PLAN_2026-04-28.md` are met |
