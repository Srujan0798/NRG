...
**Main product risk now:** local query/UI/security/service proof is strong, the managed Python suite is green, local quota-neutral C4 passes, the frontend local dependency audit is clean, frontend/reverse-proxy runtime OS scans are clean, and API package/fixable CVE scans are clean. External production claims still need deployed-environment replay, cluster C4 replay, deployed image dependency checks, production Qdrant/Redis health, founder signing, data completion for currently empty official core tables, and an API base-image resolution for strict Trivy no-fix Debian findings.
