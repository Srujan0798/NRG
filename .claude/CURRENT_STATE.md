...
**Main product risk now:** local query/UI/security/service proof is strong and local quota-neutral C4 passes, but external production claims still need deployed-environment replay, cluster C4 replay, production Qdrant/Redis health, founder signing, dependency-audit closure, and data completion for currently empty official core tables.
