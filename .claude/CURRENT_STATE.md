...
**Main product risk now:** local query/UI/security/service proof is strong, the managed Python suite is green, local quota-neutral C4 passes, and the frontend local dependency audit is clean. External production claims still need deployed-environment replay, cluster C4 replay, production image dependency checks, production Qdrant/Redis health, founder signing, and data completion for currently empty official core tables.
