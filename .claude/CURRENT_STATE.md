...
**Main product risk now:** local query/UI/security/service proof is strong, the managed Python suite is green, local quota-neutral C4 passes, the frontend local dependency audit is clean, and the frontend/API runtime images build or audit locally at the package layer. External production claims still need deployed-environment replay, cluster C4 replay, OS/base-image CVE scans, deployed image dependency checks, production Qdrant/Redis health, founder signing, and data completion for currently empty official core tables.
