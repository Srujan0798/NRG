"""End-to-End Zero-Leakage Acceptance Script."""
...
def run_acceptance_query(query: str, user_tier: int = 1) -> dict:
    """
    Run a complex multi-hop query end-to-end.

    Returns structured output with verification.
    """
    logger.info(f"Starting E2E acceptance for query: {query}")
...
def main():
    parser = argparse.ArgumentParser(description="NRG End-to-End Acceptance")
    parser.add_argument(
        "--query",
        type=str,
    ...
    logger.info("=" * 60)
    logger.info("NRG Phase 1 End-to-End Zero-Leakage Acceptance")
    logger.info("=" * 60)

    result = run_acceptance_query(args.query, args.tier)

    output_file = Path("evidence/2026-04-27/acceptance/phase1_acceptance_output.md")
    output_file.parent.mkdir(parents=True, exist_ok=True)

    with open(output_file, "w") as f:
        f.write("# Phase 1 Acceptance Output\n\n")
    ...
    logger.info(f"Acceptance output written to: {output_file}")
