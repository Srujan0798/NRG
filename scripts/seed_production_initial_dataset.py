...
def build_killer_query_results(data: dict[str, Any]) -> dict[str, Any]:
    ...
    return {
        ...
        "release_graph": data["release_graph"],
    }
...
def seed_sqlite(db_path: Path, data: dict[str, Any] | None = None) -> dict[str, int]:
    ...
    with _connect(db_path) as conn:
        conn.executescript(
            """
            DROP TABLE IF EXISTS release_funding_agencies;
            DROP TABLE IF EXISTS release_trl_progression;
            DROP TABLE IF EXISTS release_solar_seed_patents;
            DROP TABLE IF EXISTS release_iit_ai_ml_comparison;

            CREATE TABLE release_funding_agencies (
            ...
            CREATE TABLE release_trl_progression (
            ...
            CREATE TABLE release_solar_seed_patents (
            ...
            CREATE TABLE release_iit_ai_ml_comparison (
                year INTEGER NOT NULL,
                institution TEXT NOT NULL,
        ...
        return {
            "release_funding_agencies": conn.execute(
                "SELECT COUNT(*) FROM release_funding_agencies"
            ).fetchone()[0],
            "release_trl_progression": conn.execute(
                "SELECT COUNT(*) FROM release_trl_progression"
            ).fetchone()[0],
            "release_solar_seed_patents": conn.execute(
                "SELECT COUNT(*) FROM release_solar_seed_patents"
            ).fetchone()[0],
            "release_iit_ai_ml_comparison": conn.execute(
                "SELECT COUNT(*) FROM release_iit_ai_ml_comparison"
            ).fetchone()[0],
        }
...
def main(argv: list[str] | None = None) -> int:
    ...
    parser.add_argument("--database", type=Path, default=ROOT / "data" / "release_seed.db")
