  ...
  return (
    <div className={`bg-gray-50 rounded-lg p-4 ${className}`}>
      <h3 className="text-sm font-semibold text-gray-900 mb-3 flex items-center">
        ...
        {t("auto.components.CitationPanel.1")}</h3>
        ...
        {citations.map((citation, index) => (
          <div key={citation.id} className="text-sm">
            ...
            <div className="text-xs text-gray-500 mt-1">
              {t("auto.components.CitationPanel.2")}{citation.source}
              {citation.relevance_score && (
                <span className="ml-2 text-blue-600">
                  {Math.round(citation.relevance_score * 100)}{t("auto.components.CitationPanel.3")}</span>
              )}
            </div>
