...
function PhaseProgress({ currentPhase }: { currentPhase: StreamPhase | null }) {
  return (
    <div className="space-y-2">
      <div className="flex items-center gap-2 text-sm text-nrg-muted">
        <div className="w-8 h-8 rounded-full bg-saffron-100 dark:bg-saffron-900/30 flex items-center justify-center">
          {currentPhase ? (
            ...
            <span className="text-nrg-muted">⋯</span>
          )}
        </div>
...
function StreamingText({ text }: { text: string }) {
  return (
    <div className="text-sm text-nrg-text leading-relaxed whitespace-pre-wrap font-mono">
      {text}
